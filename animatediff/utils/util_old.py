...
def load_weights(
    ...
    if dreambooth_model_path != "":
        ...
        model_path = Path(dreambooth_model_path).resolve()
        if model_path.exists():
            ...
            model_path = data_dir.joinpath(dreambooth_model_path).resolve()
            logger.info(f"Loading weights from {model_path}")
        ...
        _, unet_unex = animation_pipeline.unet.load_state_dict(unet_state_dict, strict=False)
        if len(unet_unex) > 0:
            raise ValueError(f"UNet has unexpected keys: {unet_unex}")
        tenc_missing, _ = animation_pipeline.text_encoder.load_state_dict(tenc_state_dict, strict=False)
        if len(tenc_missing) > 0:
            raise ValueError(f"TextEncoder has missing keys: {tenc_missing}")
        vae_missing, _ = animation_pipeline.vae.load_state_dict(vae_state_dict, strict=False)
